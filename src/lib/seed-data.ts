...
function seedHairSalon(role: DevRole) {
  ...
  const forms = [
    ...
    { id: formGeneralId, workspaceId: WS, type: "inquiry" as const, name: "General Inquiry", fields: [{ name: "name", type: "text" as const, label: "Full Name", required: true }, { name: "email", type: "email" as const, label: "Email", required: true }, { name: "phone", type: "phone" as const, label: "Phone", required: false }, { name: "service_interest", type: "select" as const, label: "Service you're interested in", required: false, options: ["Hair", "Nails", "Lashes & Brows", "Skin", "Other"] }, { name: "message", type: "textarea" as const, label: "Your message", required: true }], branding: { primaryColor: "#8B5CF6" }, slug: "contact", enabled: true, createdAt: daysAgo(30), updatedAt: daysAgo(5) },
  ];
  useFormsStore.setState({ forms });
